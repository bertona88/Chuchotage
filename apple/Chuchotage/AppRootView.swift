...
private enum HomeMode: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .translate:
            return L10n.string("home.mode.translate", defaultValue: "Translate")
        case .conversation:
            return L10n.string("home.mode.conversation", defaultValue: "Conversation")
        }
    }
}
...
struct AppRootView: View {
    ...
    @State private var selectedHomeMode = HomeMode.translate
    ...
    private func rootContent(
        ...
        let content = VStack(spacing: rootSpacing(isCompactIOS: isCompactIOS)) {
            if shouldExpandMainContent {
                topBar

                mainContent(isCompactIOS: isCompactIOS)
    ...
    @ViewBuilder
    private func mainContent(isCompactIOS: Bool) -> some View {
        if viewModel.hasCredential {
            #if os(iOS)
            iOSHomeContent(isCompactIOS: isCompactIOS)
            #else
    ...
    private var shouldExpandMainContent: Bool {
        #if os(iOS)
        return viewModel.hasCredential && selectedHomeMode == .conversation
        #else
        return false
        #endif
    }
    ...
    #if os(iOS)
    private func iOSHomeContent(isCompactIOS: Bool) -> some View {
        VStack(spacing: isCompactIOS ? 8 : 14) {
            IOSHomeModePicker(selection: $selectedHomeMode, isCompact: isCompactIOS)

            switch selectedHomeMode {
            case .translate:
                translationSurface(isCompactIOS: isCompactIOS)
            case .conversation:
                IOSConversationTranslationScreen(viewModel: viewModel, isCompact: isCompactIOS)
            }
        }
        .frame(maxWidth: selectedHomeMode == .conversation ? 560 : .infinity)
    }
    #endif
    ...
    private func stackedTranslationSurface(isCompactIOS: Bool) -> some View {
        VStack(spacing: translationSurfaceSpacing(isCompactIOS: isCompactIOS)) {
            #if os(iOS)
            IOSOutputLanguageHomePicker(
                selectedLanguage: viewModel.targetLanguage,
                isTranslating: viewModel.isTranslating,
                isCompact: isCompactIOS,
                onSelected: { viewModel.changeTargetLanguageFromHome(to: $0) }
            )
            #endif
...
#if os(iOS)
private struct IOSHomeModePicker: View {
        ...
        HStack(spacing: 4) {
            ForEach(HomeMode.allCases) { mode in
                Button {
                    ...
                    Label(mode.title, systemImage: mode == .translate ? "waveform" : "person.2.fill")
                        .font(.system(.caption, design: .rounded, weight: .semibold))
                        .lineLimit(1)
                        .minimumScaleFactor(0.82)
                        .frame(maxWidth: .infinity)
                        ...
                        .padding(.horizontal, 8)
                        .foregroundStyle(selection == mode ? ChuchotageColor.inkDeep : ChuchotageColor.text)
                        .background(selection == mode ? ChuchotageColor.signalBlueSoft : .clear)
                        .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                }
                .buttonStyle(.plain)
...
private struct IOSOutputLanguageHomePicker: View {
    ...
    var body: some View {
        Menu {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .stroke(ChuchotageColor.signalBlueSoft.opacity(0.28), lineWidth: 1)
            )
            .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
        }
        .buttonStyle(.plain)
...
private struct IOSConversationTranslationScreen: View {
    @ObservedObject var viewModel: TranslationViewModel
    let isCompact: Bool

    @State private var activeSpeaker: ConversationSpeaker?
    @State private var localTranscript = ""
    @State private var partnerTranscript = ""
    @State private var lastObservedOutputTranscript = ""

    var body: some View {
        let localLanguage = viewModel.conversationLocalLanguage
        let partnerLanguage = viewModel.conversationPartnerLanguage
        let languagesConflict = localLanguage.code == partnerLanguage.code

        VStack(spacing: isCompact ? 7 : 10) {
            IOSConversationTranscriptPanel(
                label: L10n.string("conversation.partnerLabel", defaultValue: "Other side"),
                ...
                placeholder: conversationPlaceholder(languagesConflict: languagesConflict),
                accent: ChuchotageColor.cream,
                inputVolume: viewModel.inputVolume,
                status: viewModel.status,
                isCompact: isCompact,
                onLanguageSelected: { language in
                    updateConversationLanguages(partnerLanguageCode: language.code)
                },
                onTap: {
                    selectSpeaker(.partner, languagesConflict: languagesConflict)
                }
            )
            ...
            IOSConversationCenterLine(
                ...
                onStopTranslation: {
                    activeSpeaker = nil
                    lastObservedOutputTranscript = ""
                    viewModel.stopConversationTurn()
                }
            )

            IOSConversationTranscriptPanel(
                label: L10n.string("conversation.localLabel", defaultValue: "This side"),
                ...
                isCompact: isCompact,
                onLanguageSelected: { language in
                    updateConversationLanguages(localLanguageCode: language.code)
                },
                onTap: {
                    selectSpeaker(.local, languagesConflict: languagesConflict)
                }
            )
        }
        .frame(maxWidth: 560, maxHeight: .infinity)
        .onChange(of: activeSpeaker) {
            lastObservedOutputTranscript = viewModel.latestOutputTranscript
        }
        .onChange(of: viewModel.latestOutputTranscript) {
            appendConversationDelta(from: viewModel.latestOutputTranscript)
        }
    }

    private func conversationPlaceholder(languagesConflict: Bool) -> String {
        languagesConflict
    ...
    private func selectSpeaker(
        _ speaker: ConversationSpeaker,
        languagesConflict: Bool
    ) {
        guard !languagesConflict else { return }

        activeSpeaker = speaker
        lastObservedOutputTranscript = ""
        viewModel.startConversationTurn(speaker)
    }

    private func updateConversationLanguages(
        localLanguageCode: String? = nil,
        partnerLanguageCode: String? = nil
    ) {
        let nextLocalLanguageCode = localLanguageCode ?? viewModel.conversationLocalLanguage.code
        let nextPartnerLanguageCode = partnerLanguageCode ?? viewModel.conversationPartnerLanguage.code

        viewModel.updateConversationLanguages(
            localLanguageCode: nextLocalLanguageCode,
            partnerLanguageCode: nextPartnerLanguageCode
        )

        if nextLocalLanguageCode == nextPartnerLanguageCode {
            activeSpeaker = nil
            lastObservedOutputTranscript = ""
        ...
        lastObservedOutputTranscript = ""
        viewModel.startConversationTurn(activeSpeaker)
    }

    private func appendConversationDelta(from outputTranscript: String) {
        guard let activeSpeaker else { return }

        let delta = outputTranscript.deltaSince(lastObservedOutputTranscript)
        lastObservedOutputTranscript = outputTranscript
...
private struct IOSConversationTranscriptPanel: View {
    private static let bottomID = "ios-conversation-bottom"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: isCompact ? 7 : 9) {
            ...
            if active {
                IOSMiniSignalMeter(level: inputVolume, isActive: status == .listening)
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        Text(text.isEmpty ? placeholder : text)
                            .font(.system(isCompact ? .subheadline : .title3, design: .rounded, weight: .semibold))
                    ...
                    .frame(maxWidth: .infinity, minHeight: isCompact ? 88 : 142)
                }
                .background(ChuchotageColor.ink.opacity(0.48))
                .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
        ...
        .background(active ? ChuchotageColor.surfaceRaised.opacity(0.84) : ChuchotageColor.surface.opacity(0.64))
        .overlay(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke(borderColor, lineWidth: 1)
        )
        .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
        .accessibilityElement(children: .combine)
        .accessibilityLabel("\(label), \(language.name)")
        .accessibilityValue(statusText)
    }

    private var borderColor: Color {
        if active { return accent }
        if receiving { return ChuchotageColor.signalBlue }
        if languageConflict { return ChuchotageColor.cream.opacity(0.82) }
        return ChuchotageColor.ring.opacity(0.82)
    }
...
private struct IOSConversationLanguageMenu: View {
    ...
    var body: some View {
        Menu {
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 7)
            .background(ChuchotageColor.ink.opacity(0.52))
            .overlay(
                RoundedRectangle(cornerRadius: 8, style: .continuous)
...
private struct IOSConversationCenterLine: View {
    let status: TranslationStatus
    let activeSpeaker: ConversationSpeaker?
    ...
    var body: some View {
        ZStack {
            Rectangle()
                .fill(ChuchotageColor.ring.opacity(0.78))
                .frame(height: 1)
            ...
            HStack(spacing: 8) {
                ...
                if isRunning {
                    Button(action: onStopTranslation) {
                        Image(systemName: "stop.fill")
                            .font(.system(size: 12, weight: .bold))
                            .foregroundStyle(ChuchotageColor.inkDeep)
                            .frame(width: 30, height: 30)
                            .background(ChuchotageColor.cream)
                            .clipShape(Circle())
                    }
                    .buttonStyle(.plain)
                    .accessibilityLabel(L10n.string("translation.stop", defaultValue: "Stop translation"))
                }
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 8)
...
private struct IOSMiniSignalMeter: View {
    let level: Double
    let isActive: Bool
        ...
        GeometryReader { geometry in
            HStack(alignment: .center, spacing: 3) {
                ForEach(0..<18, id: \.self) { index in
                    Capsule()
                        .fill(visibleLevel > 0.08 ? ChuchotageColor.signalBlueSoft : ChuchotageColor.ring)
                        .frame(
                            width: 2.5,
                            height: max(4, geometry.size.height * heightFraction(at: index))
                        )
                        .opacity(visibleLevel > 0.08 ? 0.74 : 0.24)
                }
            }
    ...
    private func heightFraction(at index: Int) -> Double {
        let midpoint = 8.5
        let distance = abs(Double(index) - midpoint) / midpoint
        return min(1, 0.2 + (1 - distance) * 0.5 + visibleLevel * 0.34)
    }
}

private let iosConversationTranscriptMaxCharacters = 8_192

private extension String {
    func deltaSince(_ previous: String) -> String {
        if isEmpty { return "" }
        if hasPrefix(previous) {
            return String(dropFirst(previous.count))
        }

        let sharedPrefixLength = commonPrefix(with: previous).count
        return String(dropFirst(sharedPrefixLength))
    }

    func appendingConversationText(_ delta: String) -> String {
        let next = (self + delta).trimmingCharacters(in: .whitespacesAndNewlines)
        guard next.count > iosConversationTranscriptMaxCharacters else { return next }
